...
const createUser = async (req, res) => {
    ...
    if(files && files.length>0){
        ...
        if(!isValidpin(address.shipping.pincode))return res.status(400).send({status:false,message:"Please enter valid shipping pincode"})
        if(!isValidpin(address.billing.pincode))return res.status(400).send({status:false,message:"Please enter valid billing pincode"})
